...
class QuantityMeasurementServiceTest {
    ...
    void testCompare_EqualQuantities_ResultStringTrue() {
        QuantityMeasurementDTO result = service.compare(feetDTO, inchesDTO, testUserId);
    ...
    void testCompare_NotEqual_ResultStringFalse() {
        ...
        QuantityMeasurementDTO result = service.compare(twoFeet, inchesDTO, testUserId);
    ...
    void testCompare_DifferentCategories_ThrowsAndSavesError() {
        assertThrows(QuantityMeasurementException.class,
            () -> service.compare(feetDTO, kilogramDTO, testUserId));
    ...
    void testConvert_FeetToInches_Returns12() {
        ...
        QuantityMeasurementDTO result = service.convert(feetDTO, targetInches, testUserId);
    ...
    void testConvert_CelsiusToFahrenheit_Returns32() {
        ...
        QuantityMeasurementDTO result = service.convert(celsius, fahrenheit, testUserId);
    ...
    void testAdd_FeetPlusInches_ResultInFeet() {
        QuantityMeasurementDTO result = service.add(feetDTO, inchesDTO, testUserId);
    ...
    void testAdd_WithTargetUnit_ResultInYards() {
        ...
        QuantityMeasurementDTO result = service.add(feetDTO, inchesDTO, yardsTarget, testUserId);
    ...
    void testAdd_TemperatureUnits_ThrowsUnsupported() {
        ...
        assertThrows(QuantityMeasurementException.class,
            () -> service.add(celsius, fahrenheit, testUserId));
    }

    @Test
    void testAdd_DifferentCategories_ThrowsAndSavesError() {
        assertThrows(QuantityMeasurementException.class,
            () -> service.add(feetDTO, kilogramDTO, testUserId));
        verify(repository, atLeastOnce()).save(any());
    }
    ...
    void testSubtract_FeetMinusInches_ResultZeroFeet() {
        QuantityMeasurementDTO result = service.subtract(feetDTO, inchesDTO, testUserId);
    ...
    void testSubtract_WithTargetUnit_ResultInYards() {
        QuantityDTO yardsTarget = new QuantityDTO(0.0, QuantityDTO.LengthUnit.YARDS);
        QuantityMeasurementDTO result = service.subtract(feetDTO, inchesDTO, yardsTarget, testUserId);
        assertEquals("YARDS", result.getResultUnit());
    }
    ...
    void testDivide_FeetByFeet_ResultOne() {
        QuantityMeasurementDTO result = service.divide(feetDTO, feetDTO, testUserId);
    ...
    void testDivide_ByZero_ThrowsArithmeticException() {
        ...
        assertThrows(ArithmeticException.class,
            () -> service.divide(feetDTO, zeroInches, testUserId));
    }
    ...
    void testGetHistoryByOperation_DelegatesToRepository() {
        ...
        when(repository.findByUserIdAndOperation(testUserId, "compare")).thenReturn(List.of(entity));

        List<QuantityMeasurementDTO> result = service.getHistoryByOperation("compare", testUserId);
        assertEquals(1, result.size());
        assertEquals("compare", result.get(0).getOperation());
        verify(repository, times(1)).findByUserIdAndOperation(testUserId, "compare");
    }
    ...
    void testGetHistoryByMeasurementType_DelegatesToRepository() {
        ...
        when(repository.findByUserIdAndThisMeasurementType(testUserId, "LengthUnit")).thenReturn(List.of(entity));

        List<QuantityMeasurementDTO> result = service.getHistoryByMeasurementType("LengthUnit", testUserId);
        assertEquals(1, result.size());
        verify(repository, times(1)).findByUserIdAndThisMeasurementType(testUserId, "LengthUnit");
    }

    @Test
    void testGetOperationCount_DelegatesToRepository() {
        when(repository.countByUserIdAndOperationAndErrorFalse(testUserId, "compare")).thenReturn(3L);
        long count = service.getOperationCount("compare", testUserId);
